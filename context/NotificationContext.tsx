...
import { createCelebrationRecorder } from '@/lib/celebrationHistory';
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Persist joiners' unlock moments so they can be revisited later from the
    // Celebrations history screen even after the toast has gone.
    ...
    const unsubscribeToast = subscribe(listener);
    const unsubscribeRecorder = subscribe(recorder);
    return () => {
      unsubscribeToast();
      unsubscribeRecorder();
    };
