#!/usr/bin/env node
/**
 * Seed — inserts Let's Feast mock data. Safe to re-run (ON CONFLICT DO NOTHING).
...
try {
  ...
  await client.query(`
    INSERT INTO restaurants
      (id, name, cuisine, neighborhood, rating, num_ratings, distance,
       delivery_time, pickup_time, is_open, image_index, bg_color, lat, lng,
       allergy_tags, dietary_tags, categories, feast_window_id, member_deal)
    VALUES
      ('1','El Barrio Taco Shop','Mexican','Belmont Cragin',4.7,1243,'0.8 mi','20-30 min','10 min',true,1,'#8B2E0D',41.927500,-87.766500,'["Gluten","Dairy"]','["Halal"]','["Popular","Appetizers","Entrees","Combos","Family Meals","Drinks","Desserts"]',null,'10% off orders over $25'),
      ('2','The Logan Table','American','Logan Square',4.5,892,'1.2 mi','25-35 min','12 min',true,0,'#2C3E50',41.929500,-87.707300,'["Gluten","Dairy","Eggs"]','[]','["Popular","Appetizers","Entrees","Combos","Family Meals","Drinks","Specials"]',null,null),
      ('3','Humboldt BBQ & Soul','Soul Food','Humboldt Park',4.8,2156,'1.5 mi','30-40 min','15 min',true,2,'#6B2D0E',41.903600,-87.701000,'["Gluten","Dairy"]','["No pork"]','["Popular","Appetizers","Entrees","Family Meals","Drinks","Desserts"]',null,null),
      ('4','Wicker Park Ramen Co','Japanese','Wicker Park',4.6,1087,'2.1 mi','30-45 min','15 min',true,0,'#1A3A5C',41.908800,-87.679600,'["Gluten","Soy","Shellfish","Sesame"]','[]','["Popular","Appetizers","Entrees","Drinks","Specials"]',null,null),
      ('5','La Catrina Kitchen','Mexican','Pilsen',4.9,3421,'1.8 mi','25-35 min','10 min',true,1,'#7B1FA2',41.856700,-87.656500,'["Gluten","Dairy"]','["Halal","Vegetarian"]','["Popular","Appetizers","Entrees","Combos","Family Meals","Drinks","Specials"]',null,null),
      ('6','Randolph Street Grill','American','West Loop',4.4,756,'3.2 mi','35-50 min','20 min',true,2,'#1B5E20',41.882000,-87.648000,'["Gluten","Dairy","Eggs"]','[]','["Popular","Appetizers","Entrees","Combos","Drinks","Desserts"]',null,'$5 off first delivery'),
      ('7','South Loop Sushi Bar','Japanese','South Loop',4.7,1654,'2.8 mi','30-45 min','15 min',true,0,'#0D47A1',41.867600,-87.627000,'["Shellfish","Soy","Sesame","Fish"]','[]','["Popular","Appetizers","Entrees","Combos","Drinks","Specials"]',null,null),
      ('8','Pizza Parlor Logan','Italian','Logan Square',4.6,1923,'1.3 mi','25-35 min','12 min',true,1,'#B71C1C',41.923000,-87.708500,'["Gluten","Dairy","Eggs"]','["Vegetarian"]','["Popular","Appetizers","Entrees","Combos","Family Meals","Drinks","Desserts"]',null,null),
      ('9','Thai Lotus Garden','Thai','Wicker Park',4.5,987,'2.0 mi','30-40 min','15 min',true,2,'#4A148C',41.910000,-87.675000,'["Peanuts","Tree nuts","Shellfish","Soy","Gluten"]','["Gluten-free","Vegan"]','["Popular","Appetizers","Entrees","Combos","Family Meals","Drinks","Specials"]',null,null),
      ('10','Green Roots Cafe','Vegan','Pilsen',4.8,1342,'1.9 mi','20-30 min','10 min',true,0,'#1B5E20',41.855000,-87.660000,'["Tree nuts","Soy","Gluten"]','["Vegan","Gluten-free","High-protein"]','["Popular","Appetizers","Entrees","Combos","Family Meals","Drinks","Desserts"]',null,'Free delivery on orders over $20')
    ON CONFLICT (id) DO NOTHING
  `);

  // Backfill lat/lng for restaurants seeded before these columns existed.
  await client.query(`
    UPDATE restaurants AS r SET lat = v.lat, lng = v.lng
    FROM (VALUES
      ('1', 41.927500, -87.766500),
      ('2', 41.929500, -87.707300),
      ('3', 41.903600, -87.701000),
      ('4', 41.908800, -87.679600),
      ('5', 41.856700, -87.656500),
      ('6', 41.882000, -87.648000),
      ('7', 41.867600, -87.627000),
      ('8', 41.923000, -87.708500),
      ('9', 41.910000, -87.675000),
      ('10', 41.855000, -87.660000)
    ) AS v(id, lat, lng)
    WHERE r.id = v.id AND (r.lat IS NULL OR r.lng IS NULL)
  `);
  console.log('✓ restaurants (10)');

  // ── Feast Windows ─────────────────────────────────────────────────────────
  await client.query(`
    INSERT INTO feast_windows
      (id, restaurant_id, delivery_start, delivery_end, spots_total, spots_filled, discount, end_time)
    VALUES
  ...
  console.log('✓ feast_windows (5)');

  // ── Link restaurants → feast windows ─────────────────────────────────────
  await client.query(`
    UPDATE restaurants SET feast_window_id = 'fw1' WHERE id = '1';
    UPDATE restaurants SET feast_window_id = 'fw2' WHERE id = '3';
    UPDATE restaurants SET feast_window_id = 'fw3' WHERE id = '5';
    UPDATE restaurants SET feast_window_id = 'fw4' WHERE id = '8';
    UPDATE restaurants SET feast_window_id = 'fw5' WHERE id = '10';
  `);
  ...
  await client.query(`
    INSERT INTO menu_items
      (id, restaurant_id, category, name, description, price, allergy_tags, dietary_tags, image_index)
    VALUES
