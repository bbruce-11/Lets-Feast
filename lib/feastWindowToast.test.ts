...
describe('buildFeastWindowToast', () => {
  ...
  it('applies the savings suffix to the personalized variant even without a restaurant name', () => {
    const toast = buildFeastWindowToast(
      { type: 'feast_window_discount_unlocked', id: 'w1', discount: '7.5' },
      { hasJoined: true },
    );
    expect(toast!.icon).toBe('trophy');
    expect(toast!.message).toBe('You did it — your group deal is unlocked — save $7.50!');
  });

  it('omits the savings suffix in the personalized variant when the discount is missing or invalid', () => {
    const missing = buildFeastWindowToast(
      { type: 'feast_window_discount_unlocked', id: 'w1' },
      { restaurantName: 'Pho House', hasJoined: true },
    );
    expect(missing!.message).toBe('You did it — your group deal at Pho House is unlocked!');

    const invalid = buildFeastWindowToast(
      { type: 'feast_window_discount_unlocked', id: 'w1', discount: 'abc' },
      { restaurantName: 'Pho House', hasJoined: true },
    );
    expect(invalid!.message).toBe('You did it — your group deal at Pho House is unlocked!');
  });

  it('omits the savings suffix in the generic variant when the discount is zero or negative', () => {
    const zero = buildFeastWindowToast(
      { type: 'feast_window_discount_unlocked', id: 'w1', discount: 0 },
      { restaurantName: 'Pho House' },
    );
    expect(zero!.message).toBe('Group deal unlocked at Pho House!');

    const negative = buildFeastWindowToast(
      { type: 'feast_window_discount_unlocked', id: 'w1', discount: -3 },
      { restaurantName: 'Pho House' },
    );
    expect(negative!.message).toBe('Group deal unlocked at Pho House!');
  });
...
describe('createFeastWindowAlertListener over the shared connection', () => {
  ...
  it('keeps the generic sparkles toast for a window the user did not join', () => {
    const { conn, notify } = setup({ isJoined: () => false });
    conn.emit(JSON.stringify({ type: 'feast_window_discount_unlocked', id: 'w1', discount: '5' }));

    const [message, options] = notify.mock.calls[0];
    expect(options.icon).toBe('sparkles');
    expect(options.kind).toBe('success');
    expect(options.durationMs).toBeUndefined();
    expect(message).toBe('Group deal unlocked at Pho House — save $5.00!');
  });

  it('does not personalize when a different window than the joined one unlocks', () => {
    const { conn, notify } = setup({
      isJoined: (id) => id === 'other-window',
