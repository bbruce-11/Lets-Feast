...
export default function FeastWindowBanner({ feastWindow, onJoin, onContinue, compact = false, alreadyJoined = false }: Props) {
  ...
  const pulseScale = useSpotsPulse(spotsLeft);
    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.muted, borderColor: colors.border }]}>
        <View style={styles.header}>
          <Animated.View style={[styles.badge, { backgroundColor: colors.mutedForeground, transform: [{ scale: pulseScale }] }]}>
        ...
        <>
          ...
          <View style={styles.spotsRow}>
            ...
            <Animated.Text
              style={[
                styles.spotsText,
                { color: spotsLeft <= 3 ? colors.destructive : colors.mutedForeground },
                { transform: [{ scale: pulseScale }], alignSelf: 'flex-start' },
              ]}
            >
