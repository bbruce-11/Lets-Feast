...
export default function CartScreen() {
  ...
  const searchParams = useLocalSearchParams<{ join?: string }>();
  const pendingJoin = Array.isArray(searchParams.join) ? searchParams.join[0] : searchParams.join;
  ...
  const goToSignInForJoin = (windowId: string) => {
    router.push({ pathname: '/signin', params: { redirect: '/cart', join: windowId } } as any);
  };
      ...
      if (Platform.OS === 'web') {
        ...
        Alert.alert(
          ...
          [
            ...
            { text: 'Sign In', onPress: () => goToSignInForJoin(id) },
          ],
        );
      }
      return;
  ...
  // Resume a join the guest intended before being sent to sign in. The signin
  // screen routes back here with a `join` param; complete it once, then clear
  // the param so it can't re-fire on later visits.
  useEffect(() => {
    ...
    if (!joinedWindowIds.includes(pendingJoin)) {
      handleJoinFeastWindow(pendingJoin);
    }
  }, [pendingJoin, user]);
  ...
  const feastWindowExpired = cartNotice?.feastWindowExpired ?? false;

  const hasCartNotice = restaurantClosed || !!cartUnavailableText || !!cartPriceChangeText || feastWindowExpired;
  ...
  const CartRestoreNotice = () =>
    hasCartNotice ? (
      <View style={styles.reorderNoticeWrap}>
        ...
        {feastWindowExpired ? (
          <View style={[styles.reorderNotice, { backgroundColor: colors.gold + '12', borderColor: colors.gold + '55' }]}>
