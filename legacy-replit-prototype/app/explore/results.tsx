...
export default function ExploreResults() {
  ...
  const shownTastes = savedTastes.slice(0, 3);
  const extraTastes = savedTastes.length - shownTastes.length;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {personalized && savedTastes.length > 0 && (
          <TouchableOpacity
            onPress={() => router.push('/explore/edit')}
            activeOpacity={0.7}
            style={styles.tastesRow}
          >
            <Text style={[styles.tastesLabel, { color: colors.mutedForeground }]}>Your tastes</Text>
            <View style={styles.tastesPills}>
              {shownTastes.map((taste) => (
                <View key={taste} style={[styles.tastePill, { backgroundColor: colors.muted }]}>
                  <Text style={[styles.tastePillText, { color: colors.foreground }]}>{taste}</Text>
                </View>
              ))}
              {extraTastes > 0 && (
                <View style={[styles.tastePill, { backgroundColor: colors.muted }]}>
                  <Text style={[styles.tastePillText, { color: colors.foreground }]}>+{extraTastes}</Text>
                </View>
              )}
...
const styles = StyleSheet.create({
  ...
  tastesLabel: { fontSize: 13, fontFamily: 'Inter_600SemiBold' },
  tastesPills: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 6 },
  tastePill: {
    paddingHorizontal: 10,
    paddingVertical: 5,
    borderRadius: 14,
  },
  tastePillText: { fontSize: 13, fontFamily: 'Inter_600SemiBold' },
